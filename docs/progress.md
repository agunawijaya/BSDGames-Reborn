...
| Game | Status | Owner | Complexity | Notes |
|---|:---:|:---:|:---:|---|
| canfield | 🟢 | Agun (Kimi) | S | **Released 2026-09-21; rules/economics hardening 2026-09-24.** Canonical docs updated against `canfield.c`; `fancy-web` port at Released status (see Port Tracking) |
...
| canfield | fancy-web | 🟢 | Agun (Kimi) | — | **Released 2026-09-22; rules/economics hardening 2026-09-24.** React 18 + TypeScript + Vite. **44/44 Vitest unit tests + 6/6 Playwright e2e tests pass**; TypeScript strict; production bundle ~59 KB gzipped. Core rules aligned with `canfield.c`: empty-tableau fill order, base-rank auto-move only, hand recycle order, talon auto-refill, fourth-fruitless-pass loss. Betting economic outcome preserved with explicit Buy/Inspect/Commit phases and lazy foundation credit at Commit. CSS DOM cards (ADR-003 supersedes SVG-only constraint); SVG cheat-overlay arrows. Casino-table UI with click-to-move + drag-and-drop, visual Cheat Mode coach, flying-card animation, betting panel + live betting-info breakdown, command bar, card-counting overlay, account book, auto-save/resume, undo with $5 penalty, optional Web Audio SFX. 5 port-specific screenshots. Five port ADRs (001 tech stack; 002 cheat mode; 003 CSS cards; 004 betting phase/credit model; 005 QoL deviations). URL seed support (`?seed=N`). ✨ Complete pending: deploy live URL + daily-seed leaderboard. |
