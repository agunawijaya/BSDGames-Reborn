...
| canfield | fancy-web | 🟢 | Agun (Kimi) | — | **Released 2026-09-21.** React 18 + TypeScript + SVG cards + Vite. **23/23 Vitest unit tests + 4/4 Playwright e2e tests pass**; TypeScript strict; production bundle **54 KB gzipped**. Faithful spec engine: 52-card deal, base-card foundation rule, tableau build by alternating color, foundation wrap-around, three-card talon deal, full betting economics ($13/$13/$26/$5/$1/$1min). Casino-table UI with **click-to-move** cards, betting panel, command bar, card-counting overlay, account book (`cfscores` reinterpreted), auto-save/resume, undo with $5 penalty, optional Web Audio SFX. 4 port-specific screenshots captured via Playwright + embedded in port README. One port ADR (001 tech stack). ✨ Complete pending: deploy live URL + daily-seed leaderboard + threshold-based drag-and-drop. |
