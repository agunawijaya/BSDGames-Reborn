...
| Game | Status | Owner | Complexity | Notes |
|---|:---:|:---:|:---:|---|
| hunt | 🟠 | AntiGravity & Agun | XL | Real-time multiplayer; internet redesign. Docs done; 42 corrections against the C source pending (listed in the port’s `notes.md` §3); `fancy-web` port 🟢 Released 2026-09-24 (see Port Tracking) |
| robots | 🟢 | Agun | S | Released 2026-09-17 — canonical docs complete + `fancy-web` port at Released status; `fancy-web-remastered` released 2026-09-25 (see Port Tracking). |
...
| hunt | fancy-web | 🟢 | Agun (Claude) | — | **Released 2026-09-24.** *Hunt — Ricochet*, the first hunt port, **zero raster and audio assets** (port ADR-002, enforced by a test). `huntd` (execute/shots/expl/draw/driver/answer/makemaze) and `otto.c` ported function by function into a pure, seeded, JSON-serialisable engine that runs unchanged in Node, **matched field by field against the real daemon code** on 7 golden traces (~6,000 steps: a single-file C harness compiles the upstream `.c` files with a virtual terminal per player, since no hunt binary ships in Debian). The C won every disagreement with the canonical docs: 42 discrepancies recorded with `file:line` and proposed fixes (51×23 maze, real weapon table, mirrors that flip after every deflection, one-shot walls that regrow and throw players, ownerless mines, no ammo regen, kills-per-entry score, original authors per the man pages; canonical docs not edited). A fresh hunt arena has no mirrors at all (a perfect maze never isolates a pillar), so a braided **Ricochet** arena lets `remap()` make ~57 free-standing mirrors (chains up to 15 bounces; port ADR-006); Ricochet with Mixed bots is the default (owner's call), Classic one click away. Single player vs 1–8 bots, FFA or two teams: Classic Otto (faithful, bugs included), Novice and Sharpshooter (labelled extensions, fair: own screen only; bank shots through remembered mirrors). 10 Hz fixed tick = one `driver.c` pass (port ADR-005). Vendored Three.js r186 + custom GLSL: dark stone labyrinth lit only by hunt's own `look()` rule as a flashlight beam, blueprint memory ghosts, glass mirrors that flash and swivel, fireball/smoke/debris blasts timed along each shot's path, translucent slime, cloak heat-haze, respawn beams, bloom/CA/ACES. Coach ricochet preview (`` ` ``) and Override panel (`\`: god, infinite ammo, reveal mines, see all, freeze bots, slow motion, instant respawn; OVERRIDE ACTIVE + CHEATED score; all-off equals the seeded baseline). Classic keys + remappable Modern twin-stick scheme, 3D/split/terminal views, gunshot radar, kill feed, procedural positional sound (muted by default), reduced motion. High/Low/Lite/terminal ladder: 60 fps with 8 bots (RTX 4060), CPU-only Lite ≈13 fps under stress, no-WebGL terminal play. **90/90 node tests + 15/15 Playwright checks (GPU and CPU, 14/14 no-WebGL).** 7 port ADRs; 13 screenshots. ✨ Complete pending: listening pass on the audio, networked multiplayer (server design in port architecture.md), canonical doc fixes, live URL. |
| robots | fancy-web-remastered | 🟢 | Agun (Claude) | — | **Released 2026-09-25.** A copy of `fancy-web` with a new presentation, made at the owner's request without touching `fancy-web` (port ADR-003: rules identical — `src/game/` and its 33 tests are fancy-web's, file for file; every effect read from state changes by a tested `turnDiff` and a visual clock). **The stadium** (the owner's picture): from far away a bright star; zoom in to an arena with stands cut low on the camera side and fourteen rows behind, scrolling LED boards, three floodlight towers, and ~2,000 shader-animated fans who cheer crashes, do a Mexican wave on a ×4 chain, celebrate a cleared level with fireworks until the player presses Next level/Enter (then the hyperspace jump), and boo and throw cans, cups, bottles and paper into the arena when you lose. Sun-lit scene with shadows and a light-panel environment map; one glass deck with the cast reflected beneath; a sky shader with a new sector every level. Robots with turning heads and threat-lit visors; a two-bone-IK walk in short planted steps (tested: a planted foot never slides); wrecks as heaps of robot parts. Crashes land when the robots meet, chains in slow motion with a MELTDOWN counter; teleport arc; danger preview (`p`); Web Audio sound and crowd (`m`). 72/72 tests, strict typecheck, 338 KB gzipped. GPU: 60 fps on 40 robots with the full crowd; software WebGL drops to a low tier (8 fps; fancy-web 5 fps with 10 robots); a message instead of a blank page without WebGL. |
