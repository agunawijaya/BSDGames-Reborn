...
| Game | Status | Owner | Complexity | Notes |
|---|:---:|:---:|:---:|---|
| canfield | 🟢 | Agun (Kimi) | S | **Released 2026-09-21** — canonical docs complete + `fancy-web` port at Released status (see Port Tracking) |
...
| 🟢 Released | 7 (`robots`, `snake`, `worm`, `gomoku`, `wump`, `atc`, `trek` — `fancy-web` shipped) |
| ✨ Complete | 0 |
| **Total** | **43** |
...
| trek | fancy-web | 🟢 | Agun (Claude) | — | **Released 2026-09-22.** *Command Deep Space* identity — cinematic 2D space combat sim, sister port to `atc` in the BSD typed-command simulator collection. Canvas 2D rendering with parallax star field (3 depth layers), painted nebula gradients (rose/teal/gold/purple), programmatic ship sprites (federation cruiser silhouette Enterprise + angular bird-of-prey Klingons), phaser beam FX with glow + impact flash, photon torpedo particle trails, translucent shield bubbles. Two view modes: Tactical (current quadrant combat) + Galaxy Chart (8×8 strategic with fog-of-war). HUD panels (ship status bars, systems damage, sector contacts, bridge event log) with backdrop-blur glass panel look. Title screen with 3 difficulty presets (Novice 8/40sd, Standard 15/30sd, Expert 25/22sd). Full BSD trek command grammar: phaser/torpedo (bearing 0..12 clock face), move (course+warp), impulse, srscan/lrscan, dock, shields up/down/transfer, damages, computer, help, quit — with short aliases. Faithful engine: 8×8 galaxy of 10×10 quadrants, Klingons distributed with adjacency-safe start, starbases for full resupply on dock, stardate budget as time pressure, 8 damageable subsystems. Vanilla ES modules + Canvas 2D + zero deps + zero build. **28/28 tests pass** (engine + parser). Positioning: BSD 1980 Eric Allman origin of the FTL/Star Sector/Master of Orion genre lineage; generic Federation/Klingon terminology (not Star Trek IP). ✨ Complete pending: Klingon AI variants, bridge cutaway cinematics, Web Audio SFX, autoplay stress tests, live URL deploy, port-specific screenshots. |
...
| canfield | fancy-web | 🟢 | Agun (Kimi) | — | **Released 2026-09-21.** React 18 + TypeScript + SVG cards + Vite. **21/21 Vitest unit tests + 1 Playwright e2e drag-and-drop test pass**; TypeScript strict; production bundle **53 KB gzipped**. Faithful spec engine: 52-card deal, base-card foundation rule, tableau build by alternating color, foundation wrap-around, three-card talon deal, full betting economics ($13/$13/$26/$5/$1/$1min). Casino-table UI with **drag-and-drop + click-to-move** cards, betting panel, command bar, card-counting overlay, account book (`cfscores` reinterpreted), auto-save/resume, undo with $5 penalty, optional Web Audio SFX. 4 port-specific screenshots captured via Playwright + embedded in port README. One port ADR (001 tech stack). ✨ Complete pending: deploy live URL + daily-seed leaderboard. |
