...
| 🔴 Unclaimed | 0 |
| 🟡 Claimed | 0 |
| 🟠 In Progress | 35 (documentation phase; `dm` is docs-only with port skipped per ADR) |
| 🟢 Released | 9 (`robots`, `snake`, `worm`, `gomoku`, `wump`, `atc`, `trek`, `hangman`, `pom` — `fancy-web` shipped) |
| ✨ Complete | 0 |
| **Total** | **43** |
...
| canfield | fancy-web | 🟢 | Agun (Kimi) | — | **Released 2026-09-22.** React 18 + TypeScript + SVG cards + Vite. **29/29 Vitest unit tests + 6/6 Playwright e2e tests pass**; TypeScript strict; production bundle ~57 KB gzipped. Faithful spec engine: 52-card deal, base-card foundation rule, tableau build by alternating color, foundation wrap-around, three-card talon deal, full betting economics ($13/$13/$26/$5/$1/$1min). Casino-table UI with **click-to-move + drag-and-drop** cards, visual Cheat Mode coach (SVG arrows + glowing targets + phase recommendations), animated flying-card on foundation moves, betting panel, command bar, card-counting overlay, account book (`cfscores` reinterpreted), auto-save/resume, undo with $5 penalty, optional Web Audio SFX. 5 port-specific screenshots captured via Playwright + embedded in port README. Two port ADRs (001 tech stack; 002 cheat mode). URL seed support (`?seed=N`) for reproducible deals. ✨ Complete pending: deploy live URL + daily-seed leaderboard. |
