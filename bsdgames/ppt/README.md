# ppt

> **Format text as 8-bit paper tape — or decode paper tape back to text.**

**Category:** Utility / Display Toy
· **Status:** 🟠 In Progress
· **Original author(s):** Unknown (UC Berkeley)
· **First released:** 1988 (4.3BSD)

---

## About This Folder

This folder is the modernised port of **`ppt`** from the original BSDGames package. It is a self-contained learning artifact — code + history + design record + technical breakdown.

## Documentation

| Doc | What's in it |
|---|---|
| [`docs/about.md`](./docs/about.md) | Brochure — authors, publisher, year, cultural notes, bugs |
| [`docs/how-to-play.md`](./docs/how-to-play.md) | Manual, tips, examples |
| [`docs/architecture.md`](./docs/architecture.md) | Original C code analysis + encode/decode |
| [`docs/lessons.md`](./docs/lessons.md) | Beginner-friendly lessons drawn from the original code |
| [`docs/port-ideas.md`](./docs/port-ideas.md) | Modernisation brainstorm — UI/UX, reinterpretation |
| [`docs/spec.md`](./docs/spec.md) | Reverse-specification of mechanics |
| [`docs/notes.md`](./docs/notes.md) | Working notes |
| [`docs/manpage.md`](./docs/manpage.md) | Mirror + annotation of the shared `.6` man page |
| [`docs/diff-log.md`](./docs/diff-log.md) | Feature-by-feature original → port log |
| [`docs/test-scenarios.md`](./docs/test-scenarios.md) | Manual test scripts |
| [`docs/lineage.md`](./docs/lineage.md) | Genre siblings and modern descendants |
| [`docs/references.md`](./docs/references.md) | Sources & citations |

## Running the Port

*(Add build/run instructions once implementation exists.)*

```
$ echo "HI" | ppt
```

## Media

See [`media/`](./media/) for screenshots and demos.

## Attribution

Based on the original **`ppt`** as shipped in BSDGames. See [`../../ATTRIBUTION.md`](../../ATTRIBUTION.md).
