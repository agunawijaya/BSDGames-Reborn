...
to the `robots` `fancy-web` port.

## Existing ADRs

- [`001-tech-stack.md`](./001-tech-stack.md) — TypeScript + React
  18 + @react-three/fiber + Vite. Rationale for the fancy-web stack.
- [`002-safe-wait-deviation.md`](./002-safe-wait-deviation.md) —
  `w` key bound to safe-wait (deviation from spec-canonical risky
  wait). Rationale + how canonical behavior is still available in
  the engine module.
...
both — but only for `robots/fancy-web` specifically.
