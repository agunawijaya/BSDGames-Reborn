# AGENTS.md — `primes` (BSDGames Reborn)

This file provides context for AI agents and human contributors
working specifically on the port of **`primes`**.
...
- **One-line description:** Generate prime numbers in a given range.
- **Upstream source:**
  <https://github.com/vattam/BSDGames/tree/master/primes>
...
## 4. Design Decisions Specific to `primes`
...
- The program uses a pre-computed prime table up to 65537 and a wheel pattern for 3, 5, 7, 11, 13 to speed up sieving.
- Negative inputs are rejected with `negative numbers aren't permitted.`
- The default stop value is 2^32-1 (4294967295).
- With no arguments, `primes` reads the start value from stdin.
